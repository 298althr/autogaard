"""
Universal Scraper Windows - Enhanced Version
Updated selectors for modern Cars.com structure
"""
...
class EnhancedUniversalScraper:
    ...
    def extract_with_playwright(self, url: str, platform: str) -> dict:
        """Extract data using Playwright with enhanced selectors and debugging"""
                    ...
                    if platform == 'cars':
                        ...
                        try:
                            ...
                            data_attrs = page.evaluate("""
                                () => {
                                    const elements = document.querySelectorAll('[data-test], [data-qa], [class*="vehicle"], [class*="price"], [class*="title"]');
                                    const attrs = [];
                                    elements.forEach(el => {
                                        const test = el.getAttribute('data-test');
                                        const qa = el.getAttribute('data-qa');
                                        const className = el.className;
                                        if (test || qa || className) {
                                            attrs.push({
                                                tag: el.tagName,
                                                test: test,
                                                qa: qa,
                            ...
                            print(f"🏷️  Found {len(data_attrs)} elements with data attributes:")
                            for i, attr in enumerate(data_attrs[:10]):
                                print(f"  {i+1}. {attr['tag']}: test='{attr['test']}', qa='{attr['qa']}', class='{attr['className']}', text='{attr['text'][:50]}...'")
    ...
    def extract_data_by_platform(self, page, platform: str, url: str) -> dict:
        """Extract data based on platform-specific logic with enhanced selectors"""
        ...
        try:
            if platform == 'cars':
                base_data.update(self.extract_cars_data_enhanced(page))
    ...
    def extract_cars_data_enhanced(self, page) -> dict:
        """Extract Cars.com-specific data with enhanced modern selectors"""
        data = {'item_data': {}, 'platform_specific': {}}
        
        try:
            print("🔍 Extracting Cars.com data with enhanced selectors...")
            ...
            # Enhanced specifications extraction - comprehensive approach
            spec_patterns = [
                ('year', ['[data-test="vehicleYear"]', '.year', '[data-qa="year"]', '[class*="year"]', '.vehicle-year']),
                ('make', ['[data-test="vehicleMake"]', '.make', '[data-qa="make"]', '[class*="make"]', '.vehicle-make']),
                ('model', ['[data-test="vehicleModel"]', '.model', '[data-qa="model"]', '[class*="model"]', '.vehicle-model']),
                ('mileage', ['[data-test="vehicleMileage"]', '.mileage', '[data-qa="mileage"]', '[class*="mileage"]', '.vehicle-mileage']),
                ('transmission', ['[data-test="vehicleTransmission"]', '.transmission', '[data-qa="transmission"]', '[class*="transmission"]', '.vehicle-transmission']),
                ('engine', ['[data-test="vehicleEngine"]', '.engine', '[data-qa="engine"]', '[class*="engine"]', '.vehicle-engine']),
                ('drivetrain', ['[data-test="vehicleDrivetrain"]', '.drivetrain', '[data-qa="drivetrain"]', '[class*="drivetrain"]', '.vehicle-drivetrain']),
                ('fuel_type', ['[data-test="vehicleFuelType"]', '.fuel-type', '[data-qa="fuel-type"]', '[class*="fuel"]', '.vehicle-fuel']),
                ('exterior_color', ['[data-test="vehicleExteriorColor"]', '.exterior-color', '[data-qa="exterior-color"]', '[class*="exterior"]', '.vehicle-exterior']),
                ('interior_color', ['[data-test="vehicleInteriorColor"]', '.interior-color', '[data-qa="interior-color"]', '[class*="interior"]', '.vehicle-interior']),
                ('vin', ['[data-test="vehicleVin"]', '.vin', '[data-qa="vin"]', '[class*="vin"]', '.vehicle-vin']),
                ('stock_number', ['[data-test="vehicleStockNumber"]', '.stock-number', '[data-qa="stock-number"]', '[class*="stock"]', '.vehicle-stock']),
                ('body_style', ['[data-test="vehicleBodyStyle"]', '.body-style', '[data-qa="body-style"]', '[class*="body"]', '.vehicle-body']),
                ('condition', ['[data-test="vehicleCondition"]', '.condition', '[data-qa="condition"]', '[class*="condition"]', '.vehicle-condition'])
            ]
            ...
            print(f"❌ Enhanced Cars.com extraction error: {e}")
    ...
    def extract_images_by_platform(self, page, platform: str, url: str) -> list:
        """Extract images based on platform with enhanced selectors"""
        images = []
        
        try:
            print(f"🔍 Extracting images for {platform} with enhanced selectors...")
            
            if platform == 'cars':
                # Enhanced Cars.com image extraction
                img_selectors = [
            ...
            print(f"❌ Enhanced image extraction error: {e}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Universal Multi-Platform Scraper - Enhanced Version')
    ...
    if args.batch:
        ...
        print("Example: python universal_scraper_enhanced.py --batch links.txt")
        print("Example: python universal_scraper_enhanced.py --url 'https://example.com'")
